"""
╔══════════════════════════════════════════════════════════════════╗
║         ESH UnZipper Bot  —  database.py                       ║
║         Version  : 3.1.0                                        ║
║         Author   : @iam_esh  (Telegram)                        ║
║         Copyright: © 2026 ESH · All rights reserved            ║
║                                                                  ║
║  SQLite persistence layer — users, licences, global stats       ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
def generate_keys(count: int, duration_sec: int, label: str) -> list:
    conn = get_conn()
    keys = []
    for _ in range(count):
        ...
        keys.append(k)
    conn.execute(
        "UPDATE bot_stats SET total_keys_generated = total_keys_generated + ? WHERE id = 1",
        (count,),
    )
    conn.commit()
    conn.close()
...
def redeem_license(key: str, user_id: int):
    ...
    conn = get_conn()
    row = conn.execute(
        "SELECT * FROM licenses WHERE key = ?", (key.upper(),)
    ).fetchone()
    if not row:
        conn.close()
    ...
    return True, "[+] Licence activated!", dur
...
def get_global_stats() -> Optional[sqlite3.Row]:
    conn = get_conn()
...
def parse_duration(text: str):
    """
    Parse strings like 1D, 7D, 30D, 1H, 30M, 1W, 1MO.
    Returns (seconds: int, label: str) or (None, None) on failure.
    """
